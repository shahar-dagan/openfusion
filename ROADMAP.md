...
- **Regime-aware routing** — `router.fuse_only_with_tools` routes tool-free
  prompts to the best single model, since `bench/FINDINGS.md` shows synthesis
  only beats the best member when panelists run tools. See `examples/route.yaml.example`.
...
- **Cost preview + response cache** — estimate spend before running; dedupe
  identical prompts (needs a pluggable store, given the stateless design).
- **Client-side / multi-turn tool calling** — fuse requests that carry client
  function tools or mid-conversation tool turns (today they pass through to a
  single model); see the open question in `DESIGN.md`.
